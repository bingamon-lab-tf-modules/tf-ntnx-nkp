...
>
> The render-and-hook architecture in §1 is sound and stands. Several specifics below it do
> not. Updated 2026-07-29: now verified against the **`nkp` v2.18.0 binary**, not just the
> documentation set.
>
> | Section                         | Status                                                                                                                                                                                                                                                                                                                                    |
> | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
> | §2.1 flag contract              | **Missing `--self-managed`,** which defaults to **`false`** — meaning "a management cluster is used". As rendered, the bootstrap script targets a management cluster that does not yet exist. The management cluster must pass it; workload clusters must not.                                                                            |
> | §2.2 profile sizing / app flags | **`--app-options` and `--disable-apps` do not exist** — confirmed absent from the binary's flag set. Slim Kommander with the Small Environment installer configuration instead. Real sizing flags do exist: `--worker-replicas` (default 4), `--control-plane-replicas` (default 3), and the `--worker-*` family.                         |
> | §3 pod CIDR                     | Correct, and load-bearing — the binary's `--kubernetes-pod-network-cidr` default really is `192.168.0.0/16`, which collides with every lab VLAN and is irreversible after creation.                                                                                                                                                       |
...
> | §5 six-step upgrade contract    | **For Nutanix it is five steps, and three are wrong.** `nkp upgrade addons` accepts only `[aws, azure, gcp, preprovisioned, vsphere]` — **no `nutanix`** — so that step does not exist here. `nkp upgrade workspace` takes a _workspace_ name, not a cluster name. `nkp upgrade catalogapp` requires `<appdeployment-name> --workspace=`. |
